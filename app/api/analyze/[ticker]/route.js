...
function riskLabelFromVol(volPercent) {
  if (volPercent == null) return "Unknown";
  if (volPercent < 15) return "Low";
  if (volPercent < 25) return "Moderate";
  if (volPercent < 40) return "High";
  return "Very High";
}
...
function buildConfidence({
  ...
  const volPenalty = clamp((annualVol - 0.18) / 0.35, 0, 1) * 2.5;
  const trendScore = clamp((trendSignal + 0.12) / 0.24, 0, 1) * 2.0;
  const cagrScore = clamp((threeYearCagr + 0.08) / 0.24, 0, 1) * 2.0;
  const oneYearScore = clamp((oneYearReturn + 0.2) / 0.5, 0, 1) * 1.2;
  const sentimentScore = clamp((avgNewsSentiment + 3) / 6, 0, 1) * 0.8;

  let score = 3.0 + trendScore + cagrScore + oneYearScore + sentimentScore - volPenalty;

  if (assetType === "stock") {
    const growthQuality =
      normalizedGrowthRate != null
        ? clamp((normalizedGrowthRate + 0.05) / 0.25, 0, 1)
        : 0.5;

    const growthAgreement =
      epsGrowth != null && revenueGrowth != null
        ? 1 - clamp(Math.abs(epsGrowth - revenueGrowth) / 0.3, 0, 1)
        : 0.5;

    score += growthQuality * 1.0;
    score += growthAgreement * 0.8;
  } else {
    const stabilityBoost = clamp((0.28 - annualVol) / 0.2, 0, 1) * 1.2;
  ...
  let label = "Low";
  if (score >= 8) label = "Very High";
  else if (score >= 6.5) label = "High";
  else if (score >= 5) label = "Moderate";
  else if (score >= 3.5) label = "Cautious";
...
function buildTake({
  ...
  const strongTrend = trendSignal > 0.03;
  const weakTrend = trendSignal < -0.03;
  const goodUpside = upsidePct != null && upsidePct > 10;
  const badUpside = upsidePct != null && upsidePct < -5;

  if (assetType === "etf") {
    if (confidence.score >= 7 && goodUpside && strongTrend) {
      return "Constructive setup with solid trend support and favorable risk-adjusted upside.";
    }
    if (riskLabel === "Very High" || annualVol > 0.4) {
      return "Projection shows elevated uncertainty, so this looks more tactical than stable.";
    }
    if (weakTrend || badUpside) {
      return "Momentum is soft right now, so the near-term setup looks less attractive.";
    }
    return "Balanced ETF setup with moderate upside and manageable trend-driven risk.";
  }

  if (confidence.score >= 7 && goodUpside && strongTrend) {
    return "Attractive setup with supportive trend, decent upside, and relatively strong model confidence.";
  }
  if (riskLabel === "Very High" || annualVol > 0.4) {
    return "High volatility makes this a more speculative setup despite the projected return profile.";
  }
  if (weakTrend || badUpside) {
    return "Current signals are mixed to weak, so conviction is lower at this level.";
  }
  return "Moderate opportunity with balanced upside potential and a middle-of-the-road risk profile.";
}
...
function buildProjection({ ticker, prices, fundamentals, news }) {
  ...
  const baseUpsidePct =
    scenarioModel.fairValues?.base != null && stats.latestPrice > 0
      ? ((scenarioModel.fairValues.base - stats.latestPrice) / stats.latestPrice) * 100
      : null;

  const riskLabel = riskLabelFromVol(stats.annualVol * 100);

  const confidence = buildConfidence({
    assetType: "stock",
    annualVol: stats.annualVol,
    oneYearReturn: stats.oneYearReturn,
    threeYearCagr: stats.threeYearCagr,
    trendSignal: stats.trendSignal,
    avgNewsSentiment: stats.avgNewsSentiment,
    normalizedGrowthRate: scenarioModel.normalizedGrowthRate ?? null,
    epsGrowth,
    revenueGrowth,
  });

  const investmentTake = buildTake({
    assetType: "stock",
    confidence,
    riskLabel,
    upsidePct: baseUpsidePct,
    trendSignal: stats.trendSignal,
    annualVol: stats.annualVol,
  });
...
function buildEtfProjection({ ticker, prices, news }) {
  ...
  const baseUpsidePct =
    stats.latestPrice > 0 ? ((baseTarget - stats.latestPrice) / stats.latestPrice) * 100 : null;
